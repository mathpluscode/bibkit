...
- **Remove exact duplicates**: `python3 $TOOLS_DIR/duplicates.py <file.bib> --exact` — this comments out entries that are identical (same key, same type, same fields). Safe to auto-remove since no information is lost.
...
- **Post-fix exact duplicate removal**: `python3 $TOOLS_DIR/duplicates.py <file.bib> --exact` — entries that were different before fixing may now be identical after metadata corrections. Comment out any new exact duplicates.
